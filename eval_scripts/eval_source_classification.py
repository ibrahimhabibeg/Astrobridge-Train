...
RUN_CONFIG = {
    ...
    "suffix_tag": "source_classification_eval",
...
app = modal.App("astrobridge-source-classification-evaluation")
...
def run_evaluation_core(timestamp_dir: str, output_base_dir: str):
    ...
    from evals.data import load_test_spectra_source_classification
    from evals.responders import EvalSample, get_responder
    
    device = "cuda" if torch.cuda.is_available() else "cpu"
    task = get_task("source_classification", active_classes=RUN_CONFIG["active_classes"])
    responder = get_responder(RUN_CONFIG, device)
    
    df_test = load_test_spectra_source_classification(list(RUN_CONFIG["active_classes"].keys()))
    ...
    for batch_df in tqdm(list(chunker(df_test, batch_size)), desc="Evaluating Source Classification"):
...
def main():
    ...
    if not suffix:
        suffix = f"{RUN_CONFIG['responder_type']}_source_class"
    ...
    print(f"Starting source classification evaluation. Results will be saved to timestamped dir: {timestamp_dir}")
...
if __name__ == "__main__":
    if RUN_CONFIG["responder_type"] == "gemini":
        ...
        print("Please use `modal run eval_scripts/eval_source_classification.py` to run on Modal GPUs.")
