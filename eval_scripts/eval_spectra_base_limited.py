...
def build_mcq_prompt() -> str:
    prompt = (
        ...
        "FINAL ANSWER: X\n"
        "where X is A, B, C, D, or E. Do not explain your reasoning."
    )
    return prompt
    ...
    def predict_batch(self, samples: list[EvalSample], prompt: str) -> list[str]:
        ...
        for sample in samples:
            ...
            messages = [
                {
                    "role": "system",
                    "content": "You are an astronomer. When shown a spectrum, reply with ONLY 'FINAL ANSWER: X' where X is A, B, C, D, or E. No explanation."
                },
                {
                    "role": "user",
                    "content": [
...
def extract_final_answer(response: str) -> str:
    # The model continues from prefilled "FINAL ANSWER:", so response
    # will typically be just " B" or " B\n..." — check for a bare letter first
...
def main():
    ...
    run_evaluation.remote(model_id, prompt, timestamp_dir, batch_size=128)
