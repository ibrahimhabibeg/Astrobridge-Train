...
RUN_CONFIG = {
    ...
    "suffix_tag": "subclass_classification_eval",
    "active_classes": {
        "AGN": "AGN", 
        "STARBURST": "Starburst", 
...
app = modal.App("astrobridge-subclass-classification-evaluation")
...
def run_evaluation_core(timestamp_dir: str, output_base_dir: str):
    ...
    from evals.data import load_test_spectra_subclass_classification
    from evals.responders import EvalSample, get_responder
    
    device = "cuda" if torch.cuda.is_available() else "cpu"
    task = get_task("subclass_classification", active_classes=RUN_CONFIG["active_classes"])
    responder = get_responder(RUN_CONFIG, device)
    
    df_test = load_test_spectra_subclass_classification(list(RUN_CONFIG["active_classes"].keys()))
    ...
    for batch_df in tqdm(list(chunker(df_test, batch_size)), desc="Evaluating Subclass Classification"):
...
def main():
    ...
    if not suffix:
        suffix = f"{RUN_CONFIG['responder_type']}_subclass_class"
    ...
    print(f"Starting subclass classification evaluation. Results will be saved to timestamped dir: {timestamp_dir}")
...
if __name__ == "__main__":
    if RUN_CONFIG["responder_type"] == "gemini":
        ...
        print("Please use `modal run eval_scripts/eval_subclass_classification.py` to run on Modal GPUs.")
