...
def extract_final_answer(response: str) -> str:
    match = re.search(r"FINAL ANSWER:\s*([A-C])", response)
    if match:
...
def main():
    ...
    prompt = (
        ...
        "A: very close (z < 0.1), B: close (0.1 <= z < 0.5), C: intermediate or far (z >= 0.5). "
...
def compute_and_save_metrics(results_dir: str):    
    ...
    cm = {t: {p: 0 for p in "ABC"} for t in "ABC"}
    ...
    for t in "ABC":
        ...
        col_total = sum(cm[p][t] for p in "ABC")
    ...
    macro_f1 = sum(m["f1"] for m in per_class_metrics.values()) / 3.0
