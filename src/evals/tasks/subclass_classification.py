...
@dataclass
class SubclassClassificationPromptSpec:
    categories: List[str]
    output_format_tag: str
    options_text: str

class SubclassClassificationTask:
    name: str = "subclass_classification"

    def __init__(self, active_classes: Dict[str, str], **kwargs):
        self.active_classes = active_classes
        self.categories = list(self.active_classes.values())
        self.spec = SubclassClassificationPromptSpec(
            categories=self.categories,
            output_format_tag="FINAL ANSWER",
            options_text=", ".join(self.categories),
        )

    def get_prompt_spec(self) -> SubclassClassificationPromptSpec:
        return self.spec
    ...
    def extract_ground_truth(self, item: Any) -> str:
        # Assuming item is the raw dataset subclass string (e.g., 'AGN')
